...
class WeatherProvider(ABC):
    ...
    def __init__(self, lat, lon, cache_file=CACHE_FILE, cache_duration_minutes=CACHE_DURATION_MINUTES):
        ...
        self._data = None
        self.supplemental_providers_info = [] # To store {'instance': provider_instance, 'parameters': list_of_params}
    ...
    def _merge_supplemental_data(self, supplemental_data_all, parameters_to_merge):
        """Merges specified parameters from supplemental_data into self._data."""
                ...
                if dt_match in sup_hourly_lookup:
                    ...
                    for param in parameters_to_merge:
                        if param in sup_hour_entry:
                            ...
                            # print(f"  Merged 'hourly[dt={dt_match}].{param}'")

        # Merge daily data
        sup_daily_list = supplemental_data_all.get('daily', [])
        if sup_daily_list and self._data.get('daily'):
            # Create a lookup for supplemental daily data based on calendar date (YYYY-MM-DD)
            ...
            for primary_day_entry in self._data['daily']:
                ...
                if primary_date_str and primary_date_str in sup_daily_date_lookup:
                    ...
                    merged_something_daily = False
                    for param in parameters_to_merge:
                        if param in sup_day_entry:
                            primary_day_entry[param] = sup_day_entry[param]
                            # print(f"  Merged 'daily[dt={dt_match}].{param}'")
                            print(f"  Merged 'daily[dt={datetime.fromtimestamp(dt_match, tz=timezone.utc)}].{param}' = {sup_day_entry[param]}")
                            merged_something_daily = True
                    # if not merged_something_daily: # This debug might be too noisy if param isn't in sup_day_entry
                        # print(f"  DEBUG: No parameters from {parameters_to_merge} found in supplemental daily entry for date={primary_date_str}")
                else:
    ...
    async def fetch_data(self):
        """
        Fetches data for the primary provider, using cache if valid.
        Then, fetches data for any supplemental providers and merges specified parameters.
        """
        # _load_from_cache will now also check provider_name
        if self._is_cache_valid() and self._load_from_cache():
            return True # Data loaded from cache and matches provider
        ...
        if fetched_api_data:
            # Primary data fetched successfully, now handle supplemental providers
            self._data = fetched_api_data # Store primary data first
            ...
            for sup_info in self.supplemental_providers_info:
                ...
                if await sup_instance.fetch_data(): # This will use its own caching logic
            ...
            # self._data now contains merged data (if any supplemental fetches were successful)
            self._save_to_cache(self._data) # Save the potentially merged data
            return True
        else:
...
def get_weather_provider(config):
    ...
    # Helper to instantiate a provider by name
    def _instantiate_provider(p_name, is_supplemental=False):
        # For supplemental providers, use a different cache file to avoid conflicts if they are also used as primary
        # Or, more simply, let them use their default cache file. Their fetch_data will handle it.
        # For now, let's assume they manage their own cache independently.
    ...
    # Instantiate and attach supplemental providers
