...
class PhotoFrameGenerator:
    ...
    def generate_image(self, width, height, sort_mode="random", specific_filename=None):
        ...
        if specific_filename and specific_filename in files:
            filename = specific_filename
            logger.info(f"Processing specific photo: {filename}")
        else:
